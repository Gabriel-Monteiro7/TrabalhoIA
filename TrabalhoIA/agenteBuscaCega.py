...
def sucessoraProfundidade(valor, problemaselecionado, borda, buscaProfundidade):
    aux = expande(problemaselecionado[valor.estado])
    for item in aux:
        if(type(buscaProfundidade) == list):
...
def busca(problemaselecionado, origem, destino, tipo, buscaProfundidade):
    ...
    if(buscaProfundidade is int and tipo == "Profundidade"):
        tipo = tipo + " limitada"
    elif(type(buscaProfundidade) == list and tipo == "Profundidade"):
        buscaProfundidade.append(problemaselecionado[origem].estado)
    ...
    while(interacoes >= aux):
        ...
        while(True):
            ...
            if((borda == [] and count > 1) or count > 1000):
                ...
                return False
            if(testeObjetivo(valor.getEstado(), destino) or (valor.getProfundidade()==aux) and interacoes!=0):
                break
            else:
                if(tipo == "Largura"):
